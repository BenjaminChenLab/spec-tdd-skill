...
**Version 1.29.1** · [Protocol](skills/PROTOCOL.md) · [Changelog](CHANGELOG.md) · [License](LICENSE)
...
That is the daily surface. The fourteen skills under the hood — a verification ladder, front-ends, three delegation drivers, and a live-testing quick-fix floor — are machinery the manager invokes by name; you'll rarely type them yourself. Special shapes route themselves too (a plain bug batch goes to the lightweight multi-unit run; a money/auth/data-loss path escalates to the independent-attacker tier; an interrupted run resumes from disk). The rest of this README is what's underneath, and why each piece exists.
...
Fourteen skills — you'll type one of them. The full map, for when you want manual control: **a verification ladder + six front-ends + three parallel outer drivers — multi-task serial ([`spec-tdd-task-loop`](skills/spec-tdd-task-loop/SKILL.md)), multi-task parallel-DAG ([`spec-tdd-task-dag`](skills/spec-tdd-task-dag/SKILL.md)), and single-unit full delegation ([`spec-tdd-supervisor`](skills/spec-tdd-supervisor/SKILL.md))** — `grill-spec-tdd` (grill a fuzzy requirement, gate the spec, then route), `adversarial-grill-spec-tdd` (fuzzy **+ critical**: grill, independent auditor attacks the decisions before the gate and the final-spec test before dispatch), `spec-tdd-escalate` (route a settled requirement, no grilling), the two **plan-audit front-ends** `spec-2nd-opinion` / `spec-3rd-opinion` (independent read-only auditors verify a settled plan's facts/drift/interactions/ordering/grill coverage **before implementation**; gate on agreement — two or three opinions), the **full-pipeline front door** `spec-tdd-manager` (the whole running order in one invocation — grill → breakdown → audit → routed implementation), and `spec-tdd-quickfix` (the **live-testing quick-fix floor** below the ladder):

| Skill | Role |
|---|---|
| [`spec-tdd-quickfix`](skills/spec-tdd-quickfix/SKILL.md) | The **live-testing quick-fix floor** — below the ladder, off the acceptance-test machinery by design. The user is live-testing and streams small fixes (string/cosmetic/config swaps, tiny defects): one-sentence fix plan → the user's go → ONE **MID** subagent behind a scope fence → tiered verification (text/config = compile + the orchestrator's full diff read; logic touch = + scoped test re-run). The user's own live testing is the acceptance net — a substitution **disclosed** in the final report, never passed off as a verified run. Escape valve: a fix that grows mid-flight leaves the lane for lite or above. |
...
| `spec-tdd-quickfix` | 1 per fix (**MID**) | the fix implementer behind a scope fence; verification is the orchestrator's own diff read + compile (+ scoped tests on a logic touch) — no audit dispatch, disclosed |
...
| `spec-tdd-manager` | the routed skill's dispatches + 1 **TOP** plan audit (**2** on IRREVERSIBLE tags — 3rd-opinion) | net vs entering the driver un-split: **zero** — the plan audit replaces Phase 0's review (break down first, then audit); the grill's Phase-2 encoding audit is superseded — the invoked machinery writes and audits its own test; implementation rides eco economics (all-MID machinery + one TOP final audit per card/unit). With the **`longrun`** flag add +1 **MID** grounding digest + 1 **TOP** drafter (multi-task only) + 1 **TOP** audit runner — what shrinks is the session's context, not the token bill |

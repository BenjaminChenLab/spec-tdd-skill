...
**Version 1.8.1** · [Protocol](PROTOCOL.md) · [Changelog](CHANGELOG.md) · [License](LICENSE)
...
| [`spec-tdd-adversarial`](skills/spec-tdd-adversarial/SKILL.md) | `spec-tdd-coverage` + an **independent attacker** (a third agent context) that tries to write a wrong-but-green impl and hunts uncovered branches, then a **terminal dry-loop audit** — fresh auditors rotating lenses (test-strength / plan fidelity / deployment-ops / production quality); stops on ONE clean round, capped at 4 (5 with the `dryout` flag); a cap hit with live findings asks the human whether to continue. Top tier — critical paths only. |

...
**Version 1.8.0** · [Protocol](PROTOCOL.md) · [Changelog](CHANGELOG.md) · [License](LICENSE)
...
| [`spec-tdd-adversarial`](skills/spec-tdd-adversarial/SKILL.md) | `spec-tdd-coverage` + an **independent attacker** (a third agent context) that tries to write a wrong-but-green impl and hunts uncovered branches, then a **terminal dry-loop audit** — fresh auditors rotating lenses (test-strength / plan fidelity / deployment-ops / production quality) until 2 consecutive clean rounds, bounded at 4. Top tier — critical paths only. |

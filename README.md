...
**Version 1.21.0** · [Protocol](skills/PROTOCOL.md) · [Changelog](CHANGELOG.md) · [License](LICENSE)
...
| `spec-tdd-supervisor` | 2–3, all **MID** | the level-1 orchestrator + its nested dispatches — encoding audit + implementer on tiers above lite (3 total); the lite route: 2 (fresh review; level-1 solo-implements). **Zero TOP dispatches** — the recorded opt-in vs I19(a); the compensation is your session's final review (in-session, not a dispatch): objective gate + encoding-fidelity re-read + SPEC-DEFECT sweep. Above-band/adversarial exits to a standalone adversarial run with its own dispatch mix. |

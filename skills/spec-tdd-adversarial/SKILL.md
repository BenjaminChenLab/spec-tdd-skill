...
- **`dryout` flag** (`/spec-tdd-adversarial dryout <feature>`): args containing the token `dryout` raise the Phase-3 dry-loop cap from 4 to 5 rounds — the ONLY difference; every other rule identical. Strip the token from the feature description.
...
| Dry-loop runs unbounded "until nobody finds anything" | Prove-a-negative; that is why 2 consecutive CLEAN rounds stop the loop (capped at 4; 5 with `dryout`). Bounded dry, not exhaustion. |

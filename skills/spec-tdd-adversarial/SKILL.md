...
3. **DISPATCH THE ADVERSARIAL SUBAGENT — TOP-TIER model** (independent context; review at attack grade — I19 names the top tier for every review/attack dispatch), two parts in order:
...
- **Consolidated, after ALL units land:** dispatch the attacker ONCE over the whole hardened batch, TOP-tier, fresh contexts as always — **Part A:** the full suite + every unit's intent, NOT impls, instructed to attack unit-by-unit in dependency order and report per unit; **Part B:** branch-hunt per file cluster (one attacker per serial-chain cluster, one for the disjoint islands). This is the run's longest block — I19(e) visibility applies throughout: per-unit progress lines relayed during the block, not an end-dump.
...
- **The accepted trade (deliberate — wall-clock over earliness):** consolidation delays hole discovery until downstream units have landed on the upstream code the hole lives in. Repair is the normal loop — strengthen → re-RED → re-delegate → GREEN — then re-run the DOWNSTREAM units' acceptance specs against the fixed code (independent oracles; usually still green). Traded knowingly.
- **Terminal dry-loop: consolidated too** — one rotation over the whole batch's diff; same lenses, same severity floor, same cap, same ask-the-human on a non-converging cap.
...
| Full attack loop + dry-loop per unit on a multi-unit batch | Consolidate: per-unit Phase-3 steps 1–2 only; the attacker + dry-loop run ONCE after all units land (breaker per hole). Per-unit full cycles are the measured 5–8 h trap. |
...
- A multi-unit batch is running (or about to run) a full attack loop + dry-loop per unit — consolidate after all units land instead.

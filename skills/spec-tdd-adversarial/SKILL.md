...
- **`timebox` flag** (`/spec-tdd-adversarial timebox <feature>`): args containing the token `timebox` mark a time-constrained run and adopt the **degradation ladder** — (a) attack Parts A and B dispatch CONCURRENTLY (step 3), (b) the terminal dry-loop runs as ONE merged round (step 5), (c) the attack-loop cap drops 3 → 2, (d) Part A scopes to the change surface — the delta's entry points + the acceptance tests covering them, handed by NAME (never the diff text itself: Part A stays impl-blind), not the whole hardened suite (a multi-unit batch's own delta is its change surface; full-suite GREEN still guards the old surface). Each rung is disclosed in the final report; every verdict rule (RED/GREEN, hash, breaker semantics) is untouched. Strip the token from the feature description.
...
3. **DISPATCH THE ADVERSARIAL SUBAGENT — TOP-TIER model** (independent context; review at attack grade — I19 names the top tier for every review/attack dispatch), from this template:
   ...
   - **Part ordering:** one dispatch, Part A then Part B (one grounding pass serves both). **`timebox`:** dispatch A and B as TWO CONCURRENT dispatches — A's impl-blindness is guaranteed by its INPUT (suite + intent only), not by the sequencing; the cost is one extra grounding pass, the saving is half the wall-clock — and **scope A to the change surface**: hand it the delta's entry points + the tests covering them BY NAME (never the diff text itself — A's impl-blindness is input-guaranteed and stays so), not the whole hardened suite. Post-batching the attacker's wall-clock is reasoning-dominated, and the run's change IS the delta; wrongness in old-surface behavior the delta never touches is pre-existing, not this run's product. The full-suite GREEN runs still guard the old surface. The residual trade — a cross-cutting wrongness entering through old surface the delta only implicitly touches — is disclosed in the final report.
...
6. Surface to the user: acceptance test + property/differential tests + the attacker's final report + the dry-loop rounds' findings and residuals (not the impl) — plus a **novel-hole-shapes block**: every closed hole whose shape matches none of the Phase-1 seven classes (B1–B7) gets one structured line (shape / severity / which layer caught it / which layers missed it). Holes outside the known alphabet are the taxonomy's growth stock — fold one into the Phase-1 self-check and a whole class of future attack rounds retires (each late-found hole cost a round to discover).
...
| `timebox` rungs applied silently | Each degradation (concurrent A/B, merged dry-loop, cap 2, diff-scoped Part A) is disclosed in the final report. |

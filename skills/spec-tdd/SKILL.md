...
RETURN: implementation + unit tests + notable decisions (every deviation from the spec'd shape: symbols added/renamed/moved beyond the spec) + ACTUAL test command output proving green (not a claim).
```
...
4. **SPEC-DEFECT sweep:** diff the returned production changes against the spec/plan. For every change the spec cannot trace (helper/compat constructors, renamed public methods, logic moved beyond spec scope), ask: required by the spec, or accommodation of a test defect? When the test itself is defective, the implementer's only lever is production — it will bend production instead of reporting. Any test-defect accommodation → fix the acceptance test (it is the orchestrator's own artifact: correct it, re-hash, note the correction) and restore production to the spec'd shape. Scrutinize the implementer's "notable decisions" list specifically for this failure mode.

...
description: Use when the user says "spec-tdd-escalate", or has a SETTLED requirement (plan/spec/design doc already locked — no grilling wanted) and wants the spec-tdd tier picked for them automatically. Triggers on requirement-already-decided, auto-pick tier, skip grilling, route to spec-tdd / spec-tdd-coverage / spec-tdd-adversarial.
---

# spec-tdd-escalate

**REQUIRED BACKGROUND:** You MUST understand the `spec-tdd` family first — `spec-tdd` (general), `spec-tdd-coverage` (branch-coverage evidence), `spec-tdd-adversarial` (correctness-critical: money/auth/data-loss, independent attacker). This skill is the **no-grill front-end**: it only routes.
...
**Core principle: route-only.** You do NOT grill, do NOT write the acceptance test, do NOT delegate, do NOT gate. The tier you invoke runs its own Phase 1 (it writes the test), Phase 2 (delegates), Phase 3 (verifies). Escalate adds exactly one thing over calling a tier directly: **the machine picks the tier.**
...
## The route (pick by stakes, then invoke by name)
...
| Anything else (incl. pure refactor / no behavior change) | `spec-tdd` (default) |
...
| Over-thinks and exits the family ("just a refactor, I'll just do it") | Always pick one of the three. Pure refactor / no behavior change → `spec-tdd` (base). Don't invent a fourth path. |
...
- You picked a tier other than the three, or "none, I'll just do it."

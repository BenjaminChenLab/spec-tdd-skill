...
The requirement is already settled (a locked plan, spec, or doc). Your ENTIRE job is to **read it, run the fuzziness sniff (below), pick the tier that fits the stakes, and invoke that tier by name.** Then stop.

**Core principle: route-only.** You do NOT grill, do NOT write the acceptance test, do NOT delegate, do NOT gate. The tier you invoke runs its own Phase 1 (it writes the test) and executes from there — the delegated tiers dispatch an implementer; `spec-tdd-lite` implements in-session. Escalate adds exactly two things over calling a tier directly: **the machine picks the tier, and the fuzziness sniff refuses to silently route a doc that only looks settled** — a clean doc never triggers a question.

> Sibling front-end: `grill-spec-tdd` = requirement FUZZY → grill + gate the spec + write the test from the final spec + route. **escalate = requirement SETTLED → route only.** If you'd need to interrogate, you're in grill-spec-tdd territory, not here. (The sniff's single grill-or-route ask excepted.)
...
| Asks "should I proceed?" / gates before invoking | Full-auto — the user chose auto-routing so they don't decide. Announce the tier + a ONE-line stakes reason, then invoke. Not a briefing. (The fuzziness sniff's ONE ask — "grill or route?" — is routing hygiene on an undecided doc, not a gate.) |
...
## Red Flags — STOP
- You're about to write an acceptance test, grill the user, or ask "proceed?" — none of these are escalate's job. (The fuzziness sniff's ONE ask — "grill or route?" — is the sole exception: routing hygiene, never a grill question.)

"""Green-lie trap battery — mutation-testing layer over the benchmark.

Measures a delivered suite's DISCRIMINATING POWER, not just its oracle verdict:
an arm can pass the oracle (its impl is correct) while its test suite would let
known-wrong implementations through. Each fixture ships pre-registered trap
impls (traps/GL-XX/T*.py) — every one verified to FAIL that fixture's oracle
(selftest), i.e. genuinely wrong per the requirement. The battery swaps each
trap into an arm's run dir, runs the arm's OWN acceptance test unchanged, and
records KILLED (suite fails -> discriminating) vs SURVIVED (suite passes a
known-wrong impl). Trap-kill rate = killed / total.

Restore hygiene (the suite eats its own dog food): file-copy backup before the
swap, byte-exact copy-back after, sha256 compared before/after — any mismatch
aborts as INFRA.

Usage:
  python trap_battery.py selftest            # every trap must FAIL its oracle
  python trap_battery.py oracle A            # arms' real impls must PASS oracles
  python trap_battery.py run A               # full battery on arm A's 12 suites
  python trap_battery.py run B
  python trap_battery.py run A GL-05         # one fixture
"""
import hashlib
import os
import shutil
...
RUNS = os.path.normpath(os.path.join(HERE, "..", "..", "green-lie-runs"))

MODULES = {
    "GL-01": "retention", "GL-02": "split", "GL-03": "batch", "GL-04": "access",
    "GL-05": "intervals", "GL-06": "monthmath", "GL-07": "match", "GL-08": "netting",
    "GL-09": "allocate", "GL-10": "stale", "GL-11": "backoff", "GL-12": "makercheck",
}
...
def selftest():
    ...
    for fx in FIXTURES:
        ...
        for trap in traps_for(fx):
            total += 1
            tmp = tempfile.mkdtemp(prefix="gl-selftest-")
            try:
